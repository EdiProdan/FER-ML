...
def train(train_x, train_y, valid_x, valid_y, net, loss, config):
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(num_batches):
            ...
            if i % 5 == 0:
                print("epoch %d, step %d/%d, batch plot_loss = %.2f" % (epoch, i * batch_size, num_examples, loss_val))
            if i % 100 == 0:
...
def evaluate(name, x, y, net, loss, config):
    ...
    for i in range(num_batches):
        ...
        loss_val = loss.forward(logits, batch_y)
        loss_avg += loss_val
        # print("step %d / %d, plot_loss = %.2f" % (i*batch_size, num_examples, loss_val / batch_size))
    valid_acc = cnt_correct / num_examples * 100
    loss_avg /= num_batches
    print(name + " accuracy = %.2f" % valid_acc)
    print(name + " avg plot_loss = %.2f\n" % loss_avg)
