...
class Layer(metaclass=ABCMeta):
    ...
    def backward_inputs(self, grads):
        """
    Args:
      grads: gradient of the plot_loss with respect to the output of the layer.
    Returns:
      Gradient of the plot_loss with respect to the input of the layer.
    """
        pass
    ...
    Args:
      grads: gradient of the plot_loss with respect to the output of the layer.
    Returns:
      Gradient of the plot_loss with respect to all the parameters of the layer as a list
...
class SoftmaxCrossEntropyWithLogits():
    ...
    def forward(self, x, y):
        ...
        Returns:
          Scalar, average plot_loss over N examples.
          It is better to compute average plot_loss here instead of just sum
          because then learning rate and weight decay won't depend on batch size.

        """
        log_probs = y * scipy.special.log_softmax(x, axis=1)
        losses = np.sum(log_probs, axis=1)
    ...
    def backward_inputs(self, x, y):
        ...
        return (scipy.special.softmax(x, axis=1) - y) / x.shape[0]
...
class L2Regularizer:
    ...
    def forward(self):
        """
         Returns:
          Scalar, plot_loss due to the L2 regularization.
        """
    ...
    def backward_params(self):
        """
        Returns:
          Gradient of the L2 plot_loss with respect to the regularized weights.
        """
    ...
    def backward_inputs(self, grads):
        ...
        for i in range(m):
            for j in range(self.n):
                dq_dp[i] += j * self.w[j] * self.p[i] ** (j - 1)
        return grads * dq_dp

    def backward_params(self, grads):
        m = self.p.shape[0]
        dq_dw = np.zeros(m, self.n)
        for i in range(m):
            for j in range(self.n):
                dq_dw[i, j] = self.p[i] ** j

        return np.dot(grads, dq_dw)


#Napisite izravnu implementaciju funkcije koja obavija unaprijedni prolaz i racuna gubitak za viserazrednu
# logisticku regresiju koristeéi biblioteku numpy.
# Identificirate dijelove vaseg koda u ko- jima moze doci do numerickih pogresaka.
# Napisite robusnu implementaciju koja rjesava navedene probleme. Uputa: obratite painju na moguéi preljev u
# funkeijama exp i log.

def log_reg(x: list, y: int, W: list, b: list):
    """
    :param x: dim (D, 1)
    :param y: int
    :param W: dim (C, D)
