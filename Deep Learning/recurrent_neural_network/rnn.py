...
from utils import metrics, write_metrics, parameters

EPOCHS = 5


class VanillaRNNModel(nn.Module):
    def __init__(self, embedding_matrix, hidden_size=150, num_layers=1, bidirectional=False, dropout=0,
                 gradient_clip=0.25):
        super().__init__()

        self.rnn1 = nn.RNN(300, hidden_size=hidden_size, num_layers=num_layers, dropout=dropout,
                           bidirectional=bidirectional, batch_first=True)
        self.rnn2 = nn.RNN(hidden_size * (2 if bidirectional else 1), hidden_size=hidden_size, num_layers=num_layers,
                           dropout=dropout,
                           bidirectional=bidirectional, batch_first=True)
...
def evaluate(model, data, criterion, eval_str, epoch=None):
    ...
    path_str = str(seed)
    if epoch:
        write_metrics(acc, f1, f"../results/task_3/{path_str}.txt", epoch)
    else:
        write_metrics(acc, f1, f"../results/task_3/{path_str}.txt")
...
if __name__ == '__main__':
    ...
    model = VanillaRNNModel(embedding_matrix, num_layers=2)
