...
if __name__ == '__main__':
    ...
    ds_train_without_0 = MNISTMetricDataset(mnist_download_root, split='train', remove_class=0)
    ...
    print(f"> Loaded {len(ds_train_without_0)} training images!")
    print(f"> Loaded {len(ds_test)} validation images!")

    train_loader_without_0 = DataLoader(
        ds_train_without_0,
        batch_size=64,
        shuffle=True,
        pin_memory=True,
    ...
    train_loader_with_all = DataLoader(
        ds_traineval,
        batch_size=1,
        shuffle=False,
        pin_memory=True,
    ...
    for epoch in range(epochs):
        print(f"Epoch: {epoch}")
        t0 = time.time_ns()
        train_loss = train(model, optimizer, train_loader_without_0, device)
        ...
        if EVAL_ON_TEST or EVAL_ON_TRAIN:
            ...
            representations = compute_representations(model, train_loader_with_all, num_classes, emb_size, device)
        if EVAL_ON_TRAIN:
            print("Evaluating on training set...")
            acc1 = evaluate(model, representations, train_loader_with_all, device)
    ...
    torch.save(model.state_dict(), f"./model_without_0.pt")
